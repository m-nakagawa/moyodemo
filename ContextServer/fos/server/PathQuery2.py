...
class PathQuery2(object):
    ...
    def gen_idfr_query(self):
        ...
        if cnt > 1:
            ...
            for prefix, uri in self.namespaces.items():
                idfr_query += PathQuery2._PREFIX % (prefix, uri)
            idfr_query += PathQuery2._HEAD % self.path[0]
            for i in range(1,cnt-1):
                idfr_query += PathQuery2._BODY % (i, i+1, i+1, i+1, self.path[i])
            idfr_query += PathQuery2._LAST % (cnt,self.path[-1], cnt, cnt-1, cnt, cnt)
            idfr_query += PathQuery2._TAIL
        else:
            idfr_query = None
        return idfr_query
    ...
    def refresh_idfr(self):
        ...
        ret = PathQuery2._handle.select(self.idfr_query)
    ...
    def make_value_query(self):
        self.value_query = PathQuery2._DATA_HEAD1
        for i in range(0,len(self.idfr)):
            self.value_query += PathQuery2._DATA_HEAD2 %(i)
        self.value_query += PathQuery2._DATA_HEAD3
        for i in range(0,len(self.idfr)):
            self.value_query += PathQuery2._DATA_BODY %(i,self.idfr[i],i,i,i,i,i,i,i)
            
        self.value_query += PathQuery2._DATA_TAIL
    ...
    def update_value(self, idfr, value):
        request = PathQuery2._UPDATE % (value, idfr)
        print request
        PathQuery2._handle_update.update(request)
